...
# 학습된 파라미터 저장
print('\n학습된 파라미터를 저장합니다...')
np.save(f'cbow_params_W_in_{file_id}.npy', model.word_vecs)
np.save(f'cbow_params_W_out_{file_id}.npy', model.out_layer.params[0])
...
# 원본 텍스트도 함께 저장
with open(f'training_text_{file_id}.txt', 'w', encoding='utf-8') as f:
    f.write(text)

print('저장 완료!')
print(f'- cbow_params_W_in_{file_id}.npy')
print(f'- cbow_params_W_out_{file_id}.npy')
print(f'- word_to_id_{file_id}.npy')
print(f'- id_to_word_{file_id}.npy')
print(f'- training_text_{file_id}.txt')

# .gitignore에 패턴 추가
def update_gitignore():
    """부모 디렉토리의 .gitignore에 CBOW 파일 패턴 추가"""
    gitignore_path = '../.gitignore'
    
    patterns = [
        '# CBOW 학습 파일',
        'ch03/cbow_params_W_in_*.npy',
        'ch03/cbow_params_W_out_*.npy',
        'ch03/word_to_id_*.npy',
        'ch03/id_to_word_*.npy',
        'ch03/training_text_*.txt'
    ]
    ...
    try:
        ...
        # 이미 추가되어 있는지 확인
        if 'CBOW 학습 파일' in existing_content:
            return
